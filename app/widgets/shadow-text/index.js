...
const construct = (el) => {
    ...
    /*el.getBBox = () => {
        ...
        const leftExtra = Math.min(shadowX,0);    // will be 0 or negative
        const topExtra = Math.min(shadowY,0);    // will be 0 or negative
        const rightExtra = Math.max(shadowX,0);
        const bottomExtra = Math.max(shadowY,0);
